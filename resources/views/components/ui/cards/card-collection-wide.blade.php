<div class="container flex gap-10 relative p-4 lg:flex-col lg:items-center md:pt-24 w-full max-w-full">
    ...
    <div class="flex flex-col gap-4 lg:items-center lg:text-center">
        ...
        @if($collection->workType ?? null)
            <p class="mt-auto"><b>Тип произведений: {{$collection->workType->name}}</b></p>
        @endif
    ...
    <div class="flex flex-col justify-center gap-4 lg:w-full">
        ...
        <x-ui.link data-check-logged
                   href="{{route('account.participation.create', ['collection_id' => $collection['id']])}}">Принять
            участие
        </x-ui.link>
    </div>
