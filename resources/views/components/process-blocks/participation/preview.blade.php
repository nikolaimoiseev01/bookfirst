    ...
    <div class="flex flex-col p-4 gap-4">
        @if ($collection['status'] == \App\Enums\CollectionStatusEnums::APPS_IN_PROGRESS)
            <p class="text-dark-300 font-normal">На данный момент сборник находится в статусе приема
                ...
                23:59
                МСК в этом блоке откроется возможность скачать внутренний блок сборника, чтобы
                проверить верстку и
                внести
                комментарии</p>
        @endif
        ...
        @if ($collection['status'] == \App\Enums\CollectionStatusEnums::PREVIEW && $participation['status'] == \App\Enums\ParticipationStatusEnums::APPROVED)
            <p>На данный момент сборник находится на этапе предварительной проверки. Это означает,
                что все
                регистрационные
            ...
            <livewire:components.account.preview-comments model-type="Participation"
                                                          :model-id="$participation['id']"
                                                          comment-type="inside"/>
        @endif

        @if ($collection['status']->order() > 2 && $participation['status'] == \App\Enums\ParticipationStatusEnums::APPROVED)
            <p>На данный момент сборник был проверен авторами и можно скачать финальный макет
                внутреннего блока.</p>
            <x-ui.link href="{{$collection->getFirstMediaUrl('inside_file')}}" :navigate="false"
        ...
        @if ($collection['status'] == \App\Enums\CollectionStatusEnums::PRINT_PREPARE)
            <p class="">На данный момент мы внесли указанные исправления. Сейчас идет предпечатная
                подготовка. Печать начнется {{formatDate($collection['date_print_start'], 'j F')}}
                .</p>
        @endif
        ...
        @if (($collection['status'] == \App\Enums\CollectionStatusEnums::PREVIEW || $collection['status'] == \App\Enums\CollectionStatusEnums::PRINT_PREPARE) && $participation['status'] != \App\Enums\ParticipationStatusEnums::APPROVED)
            <p class="text-dark-300 font-normal">На данный момент сборник находится на этапе
                предварительной проверки. Вы не произвели оплату, поэтому не видите макет. На этом
                этапе еще можно произвести оплату в блоке выше, чтобы продолжить участвовать в
                сборнике.</p>
        @endif
    </div>
