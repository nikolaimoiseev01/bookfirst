    ...
    <div class="flex flex-col p-4 gap-4" x-data="{showWorks: false}">
        <div class="flex justify-between gap-4 md:flex-col">
            <div class="flex flex-col w-1/2 md:w-full">
                ...
                <p>
                    <b>Проверка: </b>{{$participation['price_check'] > 0 ? 'требуется' : 'не требуется'}}
                </p>
                <div class="flex gap-4 items-center">
                    <p><b>Произведений: </b>{{count($participation->participationWorks)}}</p>
                    <span class="text-green-400 text-xl cursor-pointer"
                          x-text="showWorks ? 'Свернуть' : 'Показать'"
            ...
            <div class="flex flex-col w-1/2 md:w-full">
                @if($participation->printOrder ?? null)
                    ...
                    <p><b>Телефон
                            получателя: </b>{{$participation->printOrder['receiver_telephone']}}</p>
                    <p><b>Адрес
        ...
        @if($collection['status']->order() < 4)
            <x-ui.link href="{{route('account.participation.edit', $participation['id'])}}">Изменить
                заявку
            </x-ui.link>
        @endif
    </div>
