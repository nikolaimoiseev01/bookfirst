<div style="width: 100%;">
    <h2 style="display: flex; align-items: center; font-size: 37px;">Правила:
        ...
        <a target="_blank" style="box-shadow: none; padding: 1px 15px; margin-top: 5px;margin-left: 20px; font-size: 18px;" href="{{route('help_account')}}#account_2" class="button">Подробная инструкция</a></h2>
        ...
        <li style="margin-bottom: 10px; list-style-type: disc;"><p>Текст нежирный (внутри текста нельзя вставлять жирные символы)</p></li>
    ...
    <p>Если все правила учтены, мы готовы анализировать Ваш файл :)</p><br>
        <div style="margin-top: 20px; margin-right: 20px;" class="input-file">
            ...
            <input accept=".docx" style="display: none;" wire:model="file" name="work_file" class="custom-file-input" id="work_file"
                   type="file"/>
    ...
    @if (count($works) > 0)
        ...
        <p style="margin-bottom: 20px; font-size: 24px;"><i>Каждое произведение можно отредактировать или удалить в случае ошибки автоматического анализа.</i></p>
        ...
        <div class="works-prev">
            @foreach($works as $work)
                <div id="prev_work_{{$loop->index}}" class="container work-prev">
                        <span style="right: 40px;" id="change_{{$loop->index}}" class="change-work-prev tooltip" title="Редактировать">
                    ...
                    <span id="delete_{{$loop->index}}" wire:click.prevent="delete_work({{$loop->index}})"
                        ...
                        <svg id="Слой_1" data-name="Слой 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                            <path d="M256,512C114.84,512,0,397.16,0,256S114.84,0,256,0,512,114.84,512,256,397.16,512,256,512Zm0-475.43C135,36.57,36.57,135,36.57,256S135,475.43,256,475.43,475.43,377,475.43,256,377,36.57,256,36.57Z" transform="translate(0 0)"/><path d="M347.43,365.71a18.22,18.22,0,0,1-12.93-5.35L151.64,177.5a18.29,18.29,0,0,1,25.86-25.86L360.36,334.5a18.28,18.28,0,0,1-12.93,31.21Z" transform="translate(0 0)"/><path d="M164.57,365.71a18.28,18.28,0,0,1-12.93-31.21L334.5,151.64a18.29,18.29,0,0,1,25.86,25.86L177.5,360.36A18.22,18.22,0,0,1,164.57,365.71Z" transform="translate(0 0)"/>
                        </svg>
                    </span>
                    ...
                    <input style="    font-size: 24px; display:none; margin: 10px 40px 0 10px;" type="text" value="{{$work['title']}}"
                           name="title_input_{{$loop->index}}" id="title_input_{{$loop->index}}">
                    <p style="max-height: 300px; overflow: auto; margin:10px;" id="text_{{$loop->index}}">{!! $work['text'] !!}</p>
                    <div class="editable_div" id="text_input_{{$loop->index}}"
                         style="max-height: 300px; overflow: auto; height: 100%; margin:10px; display:none;" id="text" contenteditable="true">
                        <p>{!! $work['text'] !!}</p>
                    </div>
