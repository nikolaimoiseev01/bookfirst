<form x-data="{
    ...
    <div class="flex container p-8 mb-8">
        ...
        <div class="min-w-[20%] max-w-[25%] flex border-l border-dark-100 pl-8">
                ...
                <div class="flex flex-col items-center my-auto">
                    ...
                    @if($promocode)
                        <p class="text-dark-200 text-lg italic text-center">Промокод <b>{{$promocode['name']}}</b>
                            применен!
                            <br>Теперь в цене учитывается <b>скидка в {{$promocode['discount']}}%</b></p>
                    @else
                        <div x-show="hasPromo" class="flex flex-col">
                            <x-ui.input.text name="Имя" class="!text-lg !py-0" placeholder="Промокод"
                                             wire:model="promocodeInput"/>
                            <div class="flex items-center justify-center gap-2">
                                <x-ui.tooltip-wrap wire:click="checkPromo()" text="Проверить прокод">
            ...
            @else
                <p class="font-black text-2xl text-center my-auto text-dark-200">Добавьте произведения, чтобы начать
                    считать стоимость</p>
            @endif
        </div>
    </div>
