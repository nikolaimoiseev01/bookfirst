<div class="part vote_block_wrap">
    ...
    <div class="block_wrap container">
        ...
        <div class="info_wrap">
            ...
            @elseif ($collection['col_status_id'] >= 2 && ($participation['pat_status_id'] <= 2 || $participation['pat_status_id'] === 99))
                {{-- Если сборник поехал, но клиент не прошел оплату --}}
                <p class="no-access">Из-за отсутствия оплаты Вы не были включены в список участников.</p>

            @elseif($collection['col_status_id'] >= 2)
                {{-- Если сборник в предварительной проверке --}}

                @if(count($winners) === 0)
                    {{-- Если еще не выбраны победители--}}

                    @if(!($voted_to['user_id'] ?? null))
                        {{-- Если еще не проголосовал --}}
