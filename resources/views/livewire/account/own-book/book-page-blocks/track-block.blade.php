<div class="part track_block_wrap" id="print_part">
    ...
    <div class="block_wrap container">
        ...
        <div class="info_wrap">
            @if ($own_book['own_book_status_id'] === 99)
                <p class="no-access">
                    Данное издание больше неактуально. Если у вас возникнут вопросы, почему появился такой статус, вы
                    все еще можете использовать чат выше.
                </p>
            @else
                @if($own_book->printorder ?? null)
                    ...
                    @if ($own_book['own_book_status_id'] < 4)
                        {{-- Если еще работаем с макетами --}}
                        <p class="no-access">
                            В вашем издании включены печатные экземпляры, но процесс оплаты и печати будет доступен
                            только
                            после
                            утверждения внутреннего блока и обложки.
                        </p>
                    @elseif ($own_book['own_book_status_id'] === 4)
                        {{-- Если закончили работу с макетами --}}
                        <p class="no-access">
                    ...
                    @elseif ($own_book['own_book_status_id'] === 5)
                        {{-- Если уже была оплата, ждем в печать --}}
                        <p class="no-access">
                    ...
                    @elseif ($own_book['own_book_status_id'] === 6)
                        {{-- Если идет печать --}}
                        <p class="no-access">
                            Оплата успешно подтверждена! Прямо сейчас идет печать книги. Обычно она занимает 11-13
                            рабочих
                            дней. Как только экземпляры будут
                    ...
                    @else
                        {{-- Если все завершено, была печать, и пересылка оплачена --}}
                        <p>
                ...
                @else
                    {{-- Если нет печатных экземпляров --}}
                    @if ($own_book['own_book_status_id'] < 9)
                        {{-- Если еще не завершили работу --}}
                        <p class="no-access">
                            У Вас нет заказа печатных экземпляров. Вы сможете заказать печать, но процесс печати будет
                            доступен
                    ...
                    @elseif ($own_book['own_book_status_id'] == 9)
                        <p class="no-access">
                            Книга успешно издана! У Вас не было заказа печатных экземпляров, поэтому в этом блоке ничего
                            нет.
                            Вы можете заказать печать книги, написав в чат по этому изданию.
                        </p>
                    @endif
                @endif
            @endif
